...
class mod_elang_report_form extends moodleform
{
    ...
    public function definition() {
        ...
        // Group.
        $dbgroups = $DB->get_records('groups', array('courseid' => $COURSE->id));
...
// Get the user id.
$iduser = optional_param('id_user', 0, PARAM_INT);
...
if ($format == 'csv') {
    ...
    if (version_compare($version, '2.7') < 0) {
        add_to_log(
            ...
            'elang',
            'view report',
            'mod/elang/view.php?id=' . $cm->id . '&format=csv',
            'csv',
            $cm->id,
            $USER->id
        );
    ...
    header('Content-disposition: attachment; filename="' .
        Elang\generate_title($elang, json_decode($elang->options, true)) . ' ' . date('Y-m-d H:i:s') .
        '.csv"'
    );
    ...
    // Get the group id.
    $idgroup = optional_param('id_group', 0, PARAM_INT);
    ...
    foreach ($answers as $answer) {
        if (isset($users[$answer->id_user])) {
            ...
            foreach ($json as $n => $data) {
                ...
                } else if ($data['content'] == $solutions[$answer->id_cue]->json[$n]) {
    ...
    if ($iduser) {
        if (version_compare($version, '2.7') < 0) {
            ...
            add_to_log(
                ...
                'elang',
                'view report',
                'mod/elang/view.php?id=' . $cm->id . '&id_user=' . $iduser,
                'user ' . $iduser,
                $cm->id,
                $USER->id
            );
            ...
            $params = array(
                'context' => $context,
                'courseid' => $course->id,
                'other' => array('action' => 'one', 'id_user' => $iduser)
            );
            $event = \mod_elang\event\report_viewed::create($params);
            $event->trigger();
        }
        ...
        if (is_enrolled($context, $iduser, 'mod/elang:isinreport')) {
            $student = $DB->get_record('user', array('id' => $iduser), 'firstname, lastname, email');
            ...
            // Get the answers.
            $answers = $DB->get_records('elang_users', array('id_elang' => $cm->instance, 'id_user' => $iduser));
                ...
                foreach ($solution->json as $n => $data) {
                    ...
                    if (isset($answers2[$cue][$n])) {
                        if ($answers2[$cue][$n]['help']) {
                            $help++;
                        } else if ($answers2[$cue][$n]['content'] == $data) {
                            $success++;
        ...
        if ($fromform = $mform->get_data()) {
            ...
            $idgroup = $fromform->group;

            // Store the data in the session.
            $cache->set('perpage', $perpage);
            $cache->set('id_group', $idgroup);
        } else {
        ...
        if ($perpage > 0) {
            ...
            echo $OUTPUT->paging_bar(
                count_enrolled_users($context, 'mod/elang:isinreport', $idgroup),
                $page,
                $perpage,
